...
/**
 * See https://github.com/DhyanB/Open-Imaging/blob/master/src/main/java/at/dhyan/open_imaging/GifDecoder.java
 * @author Debasis
 */
public class GIFDecoder {
	...
	static final class CodeTable {
		private final byte[][] tbl; // Maps codes to lists of colors
		...
		private final int add(final byte[] indices) {
			if (nextCode < 4096) {
		...
		private final void init(final GifFrame fr, final byte[] activeColTbl, final BitReader br) {
			this.br = br;
			...
			tbl[fr.clearCode] = new byte[] { fr.clearCode }; // CLEAR
			tbl[fr.endOfInfoCode] = new byte[] { fr.endOfInfoCode }; // EOI
	...
	final class GifFrame {
		...
		private byte[] localColTbl; // Local color table (optional)
		// Image data
		private int firstCodeSize; // LZW minimum code size + 1 for CLEAR & EOI
		private byte clearCode;
		private byte endOfInfoCode;
	...
	public final class GifImage {
		...
		public byte[] globalColTbl; // Global color table
		...
		private final byte[] decode(final GifFrame fr, final byte[] activeColTbl) {
			...
			byte[] pixels = tbl[code]; // Output pixel for first code
			...
			try {
				while (true) {
					...
					final byte[] prevVals = tbl[prevCode];
					final byte[] prevValsAndK = new byte[prevVals.length + 1];
		...
		private final byte[] deinterlace(final byte[] src, final GifFrame fr) {
		...
		private final void drawFrame(final GifFrame fr) {
			...
			final byte[] activeColTbl = fr.hasLocColTbl ? fr.localColTbl : globalColTbl;
			// Get pixels from data stream
			byte[] pixels = decode(fr, activeColTbl);
			if (fr.interlaceFlag) {
				pixels = deinterlace(pixels, fr); // Rearrange pixel lines
			}
			// Create image of given type for frame area
			final BufferedImage frame = new BufferedImage(fr.w, fr.h, BufferedImage.TYPE_3BYTE_BGR);
			arraycopy(pixels, 0, ((DataBufferByte) frame.getRaster().getDataBuffer()).getData(), 0, fr.wh);
			...
			prevPx = new byte[wh];
			arraycopy(((DataBufferByte) img.getRaster().getDataBuffer()).getData(), 0, prevPx, 0, wh);

			// Create another copy for the end user to not expose internal state
			fr.img = new BufferedImage(w, h, BufferedImage.TYPE_3BYTE_BGR);
			arraycopy(prevPx, 0, ((DataBufferByte) fr.img.getRaster().getDataBuffer()).getData(), 0, wh);
			...
			if (fr.disposalMethod == 2) {
				...
				arraycopy(prevPx, 0, ((DataBufferByte) img.getRaster().getDataBuffer()).getData(), 0, wh);
			}
		}
	...
	public static final GifImage read(final byte[] in) throws IOException {
		...
		if (img.hasGlobColTbl) {
			img.globalColTbl = new byte[img.sizeOfGlobColTbl];
		...
		while (pos < in.length) {
			...
			case 0x2C: // Image descriptor
				...
				if (frame.hasLocColTbl) {
					frame.localColTbl = new byte[frame.sizeOfLocColTbl];
	...
	static final int readColTbl(final byte[] in, final byte[] colors, int i) {
	...
	static final int readImgData(final GifFrame fr, final byte[] in, int i) {
		...
		final byte clearCode = (byte)(1 << minCodeSize); // CLEAR = 2^minCodeSize
		fr.firstCodeSize = minCodeSize + 1; // Add 1 bit for CLEAR and EOI
		fr.clearCode = clearCode;
		fr.endOfInfoCode = (byte)(clearCode + 1);
